...
export const Philosophy = () => {
  ...
  const renderWords = (text, highlightWord = '') => {
    return text.split(' ').map((word, i) => {
      ...
      return (
        <span 
          key={i} 
          className={`inline-block mr-2 philosophy-word opacity-0 translate-y-5 ${isHighlight ? 'text-blue-400' : ''}`}
        >
          {word}
        </span>
      );
  ...
  return (
    <section ref={containerRef} className="bg-gray-900 w-full relative overflow-hidden">
      {/* SVG Texture */}
      ...
      <div className="max-w-3xl mx-auto text-center py-32 px-6 relative z-10">
        ...
        {/* Stats Row */}
        <div className="stats-row border-t border-gray-700 mt-24 pt-12 flex flex-col md:flex-row justify-center items-center gap-12 md:gap-24">
          <div className="stat-item flex flex-col items-center">
            <span className="font-mono text-3xl font-bold text-white mb-2">92%</span>
            <span className="text-xs text-gray-500 uppercase tracking-wider font-semibold">Avg. Retention</span>
          </div>
          <div className="stat-item flex flex-col items-center">
            <span className="font-mono text-3xl font-bold text-white mb-2">15m</span>
            <span className="text-xs text-gray-500 uppercase tracking-wider font-semibold">Daily Queue Time</span>
          </div>
          <div className="stat-item flex flex-col items-center">
            <span className="font-mono text-3xl font-bold text-white mb-2">2.4M</span>
            <span className="text-xs text-gray-500 uppercase tracking-wider font-semibold">Intervals Logged</span>
          </div>
        </div>
      </div>
    </section>
